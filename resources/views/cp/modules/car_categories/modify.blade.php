	...
	<div class="container-fluid">
		<div class="panel panel-headline">
			<div class="panel-heading">
				<h3 class="panel-title">Modify car category</h3>			
			</div>
			<div class="panel-body">
				@include('partials.success')
				<form method="POST" class="form-auth-small" enctype="multipart/form-data" action="{{ route('cp.modules.car_categories.update', og($carcategory, 'id')) }}">
                    @csrf
						<div>
						  	<!-- Nav tabs -->
							    	...
							    	<div role="tabpanel" class="tab-pane fade{{ lang() == $local ? ' in active' : '' }}" id="{{ $local }}">
							    		<div class="form-group">
					                        ...
					                        <input 
					                        	...
					                        	id="{{$local}}[title]" 
					                        	value="{{ og($carcategory->translate($local), 'title') }}" 
					                        	placeholder="Title"
					                        >
                    ...
                    <button type="submit" class="btn btn-primary btn-lg btn-block">MODIFY NEW</button>
                </form>
			</div>
		</div>
	</div>
